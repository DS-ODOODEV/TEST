...
class WizardReportHrSlips(models.TransientModel):
    _name = 'wizard.report.hr.slips'
    _description = 'Wizard Report HR Slips'

    date_from = fields.Date('From', required=True, default=lambda *a: time.strftime('%Y-%m-01'))
    date_to = fields.Date('To', required=True, default=str(datetime.now() + relativedelta.relativedelta(months=+1, day=1, days=-1))[:10])
    employee_ids = fields.Many2many('hr.employee', 'rel_wizard_hr_slips', 'employee_id', 'wizard_id', string='Employee', required=False)
    ...
    def get_slips(self):
        slip_ids = False
        for rec in self:
            args = [('state','not in', ('draft', 'cancel')), ('company_id', '=', rec.company_id.id)]
            if rec.date_from:
                args.append(('date_from', '>=', rec.date_from))
            if rec.date_to:
                args.append(('date_to', '<=', rec.date_to))
            if rec.employee_ids:
                args.append(('employee_id', 'in', rec.employee_ids.ids))
            slip_ids = self.env['hr.payslip'].search(args)
        return slip_ids
    ...
    def action_generate_excel(self):
        ...
        for rec in self:
            ...
            values = self.generate_values(slips=slip_ids, report=rec.report_id)
            #Tittles
            ...
            sheet.merge_range(1, 1, 1, (len_ids + x_header ), (("Planilla de beneficios a empleados correspondiente del %s al %s") %(rec.date_from.strftime('%d/%m/%y'), rec.date_to.strftime('%d/%m/%y'))), company_subtittle_style)
            ...
            return {
                ...
                'url': 'web/content/?model=wizard.report.hr.slips&field=file&download=true&id=%s&filename=nominas.xlsx' % (rec.id),
                'target': 'new',
            }
