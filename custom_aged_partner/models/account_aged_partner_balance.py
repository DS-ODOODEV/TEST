...
class ReportAccountAgedPartner(models.AbstractModel):
    ...
    def _get_columns_name(self, options):
        ...
        columns.append({'name': _("Importe en USD"), 'class': 'date', 'style': 'white-space:nowrap;'})
        return columns
        ...
        for values in results:
            vals = {
                ...
                'columns': [{'name': ''}] * 4 + [{'name': self.format_value(sign * v), 'no_format': sign * v}
                           ...
                           + [{'name': self.format_value(sign * values['total_usd'], usd_currency),
                               'no_format': sign * values['total_usd']}],
                'trust': values['trust'],
                'unfoldable': True,
                        ...
                        'columns': [{'name': v} for v in
                                   ...
                                   [{'name': self.format_value(sign * v, blank_if_zero=True), 'no_format': sign * v} for
                                    v in [line['period'] == 6 - i and line['amount'] or 0 for i in range(7)]] + [
                                       {'name': self.format_value(sign * line['amount_usd'], usd_currency),
                                        'no_format': sign * line['amount_usd']}],
                        'action_context': {
        ...
        if total and not line_id:
            total_line = {
                ...
                'columns': [{'name': ''}] * 4 + [{'name': self.format_value(sign * v), 'no_format': sign * v} for v in
                                                 [total[7], total[4], total[3], total[2], total[1], total[0],
                                                  total[5]]] +
                           [{'name': self.format_value(sign * total[6], usd_currency), 'no_format': sign * total[6]}],
            }
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move, period_length):
        ...
        for i in range(5):
            ...
            for line in move_lines:
                ...
                if partner_id not in partners_amount:
                    ...
                    partners_amount_usd[partner_id] = 0.0
                ...
                if line.currency_id != usd_currency:
                    line_amount_usd = line.company_id.currency_id._convert(line.balance, usd_currency, user_company,
                                                                           date_from)
                else:
                    line_amount_usd = line.amount_currency
                ...
                for partial_line in line.matched_debit_ids:
                    if partial_line.max_date <= date_from:
                        ...
                        if line.currency_id != usd_currency:
                            line_amount_usd += partial_line.company_id.currency_id._convert(partial_line.amount,
                                                                                            usd_currency,
                                                                                            user_company, date_from)
                        else:
                            line_amount_usd += partial_line.amount_currency
                ...
                if not self.env.company.currency_id.is_zero(line_amount):
                    ...
                    partners_amount_usd[partner_id] += line_amount_usd
            ...
            history_usd.append(partners_amount_usd)
        ...
        for line in self.env['account.move.line'].browse(aml_ids):
            ...
            if partner_id not in undue_amounts:
                ...
                undue_amounts_usd[partner_id] = 0.0
            line_amount = line.company_id.currency_id._convert(line.balance, user_currency, user_company, date_from)
            if line.currency_id != usd_currency:
                line_amount_usd = line.company_id.currency_id._convert(line.balance, usd_currency, user_company, date_from)
            else:
                line_amount_usd = line.amount_currency
            ...
            for partial_line in line.matched_debit_ids:
                if partial_line.max_date <= date_from:
                    line_amount += partial_line.company_id.currency_id._convert(partial_line.amount, user_currency,
                                                                                user_company, date_from)
                    if line.currency_id != usd_currency:
                        line_amount_usd += partial_line.company_id.currency_id._convert(partial_line.amount, usd_currency,
                                                                                    user_company, date_from)
                    else:
                        line_amount_usd += partial_line.amount_currency
            ...
            if not self.env.company.currency_id.is_zero(line_amount):
                ...
                undue_amounts_usd[partner_id] += line_amount_usd
                lines.setdefault(partner_id, [])
                lines[partner_id].append({
                    'line': line,
                    'amount': line_amount,
        ...
        for partner in partners:
            ...
            if partner['partner_id'] in undue_amounts:  # Making sure this partner actually was found by the query
                ...
                undue_amt_usd = undue_amounts_usd[partner['partner_id']]
            ...
            values['total_usd'] = sum([undue_amt_usd] + [history_usd[i][partner['partner_id']] for i in range(5) if
                                                         partner['partner_id'] in history_usd[i]])
            # Add for total
            total[(i + 1)] += values['total']
            total[(i + 2)] += values['total_usd']
