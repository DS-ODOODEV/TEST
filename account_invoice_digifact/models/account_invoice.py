    ...
    def generate_xml(self):
        ...
        for inv in self:
            ...
            if inv.is_export:
                ...
                complement['direccion_consignatario'] =  inv.company_id.street + ' ' + inv.company_id.street2 if inv.company_id.street and inv.company_id.street2 else ''
                complement['incoterm'] =  inv.invoice_incoterm_id.code
                dte['complementos'] = complement
            #Items de la factura
            for line in inv.invoice_line_ids:
                ...
                #Taxes calculted
                item['grabable'] = str(round(taxes.get('total_included', 0.00), 3))
                item['subtotal'] = str(round(taxes.get('total_included', 0.00), 3))
                item['descuento'] = str(round((discount_unit * line.quantity), 3))
                item['cantidad'] = str(line.quantity)
                item['descripcion'] = str(line.name)
                item['preciounitario'] = str(round(taxes_unit.get('total_included', 0.00),3))
                item['uom'] = 'UNI'
                ...
                for tax in taxes.get('taxes', False):
                    tax_name = ""
                    subtotal_taxes += round(tax.get('amount', 0.00), 3)
                    ...
                    tax_line = {
                        'base': str(round(tax.get('base', 0.00), 3)),
                        'tax': str(round(tax.get('amount', 0.00), 3)),
                        'tax_name': tax_name,
            ...
            dte['total'] = str(round(inv.amount_total, 3))
        return dte
