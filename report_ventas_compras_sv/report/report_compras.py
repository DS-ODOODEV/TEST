...
class ReportPurchaseBook(models.AbstractModel):
    _name = 'report.report_ventas_compras_sv.report_purchase_book'
    ...
    def generate_records(self, data, company_id, rows_per_page):
        ...
        facturas = self.env['account.move'].search(
            [('state', 'in', ['posted']),
             ...
             ('date', '<=', date_to),
             ('invoice_date', '!=', False),
             ('company_id', '=', compania[0])], order='invoice_date,name')
...
class ReportPurchaseBookGuatemala(ReportPurchaseBook):
    _name = 'report.report_ventas_compras_sv.report_purchase_book_guatemala'
