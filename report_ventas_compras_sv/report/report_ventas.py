...
class ReportSaleBook(models.AbstractModel):
    _name = 'report.report_ventas_compras_sv.report_sale_book'
        ...
        for inv in facturas:
            ...
            for line in inv.invoice_line_ids:
                ...
                if line.product_id.tipo_gasto == "compra":
                    if not inv.is_export and 'exportación' not in inv.journal_id.name.lower():
                        if line.tax_ids:
...
class ReportSaleBookGuatemala(ReportSaleBook):
    _name = 'report.report_ventas_compras_sv.report_sale_book_guatemala'


class ReportSaleBookCF(ReportSaleBook):
    _name = 'report.report_ventas_compras_sv.report_sale_cf_book'

    def generate_records(self, data, company_id, rows_per_page, cf=False):
        result, linea = super(ReportSaleBookCF, self).generate_records(data, company_id, rows_per_page, True)
        grouped_result = {}
        for row in result:
