...
class WizardVentasCompras(models.TransientModel):
    _name = 'wizard.ventas.compras.sv'
    ...
    journal_ids = fields.Many2many(
        'account.journal', 'book_journal_sv_rel', 'wizard_id', 'journal_id',
    ...
    type_report = fields.Selection(
        ...
        required=True,
        default="pdf")
    difference = fields.Many2one('res.users')
    ...
    def print_report(self):
        ...
        datas['form'] = self.read(['company_id', 'journal_ids', 'tax_id',
                                   ...
                                   'date_to'])[0]
        report_name = 'report_ventas_compras_sv.report_purchase_book'
        if self.type_report == 'xls' and self.type_book == 'purchase':
            report_name = 'report_ventas_compras_sv.purchase_book_xls'
        elif self.type_report == 'xls' and self.type_book == 'sale':
            report_name = 'report_ventas_compras_sv.sale_book_xls'
        elif self.type_book == 'sale':
            report_name = 'report_ventas_compras_sv.report_sale_book'
    ...
    def print_report_sale(self):
        ...
        if company.country_id.name == 'Guatemala':
            return self.env.ref('report_ventas_compras_sv.report_sale_book_ab_guatemala').report_action(self, data=datas)
        else:
            return self.env.ref('report_ventas_compras_sv.report_sale_book_ab').report_action(self, data=datas)
    ...
    def print_report_purchase(self):
        ...
        company = self.env['res.company'].search([('id', '=', data['company_id'][0])])
        if company.country_id.name == 'Guatemala':
            return self.env.ref('report_ventas_compras_sv.report_purchase_book_ab_guatemala').report_action(self, data=datas)
        else:
            return self.env.ref('report_ventas_compras_sv.report_purchase_book_ab').report_action(self, data=datas)
    ...
    def print_report_ventas_cf(self):
        ...
        return self.env.ref('report_ventas_compras_sv.report_sale_cf_book_ab').report_action(self, data=datas)
